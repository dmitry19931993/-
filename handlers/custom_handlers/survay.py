...
def survay(message: Message) -> None:
    bot.set_state(message.from_user.id, UserInfoState.name, message.chat.id)
    bot.send_message(message.from_user.id, f'Привет,{message.from_user.fullname}, введи свое имя')
...
def get_age(message: Message) -> None:
    if message.text.isdigit():
        ...
        bot.set_state(message.from_user.id, UserInfoState.contry, message.chat.id)
...
@bot.message_handler(state= UserInfoState.contry)
def get_contry(message: Message) -> None:
    if message.text.isalpha():
        bot.send_message(message.from_user.id, 'Спасибо, записал. Теперь введи свой город: ')
        bot.set_state(message.from_user.id, UserInfoState.city, message.chat.id)

        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data['contry'] = message.text
    else:
...
@bot.message_handler(content_types= ['text', 'contact'], state= UserInfoState.telephone_number)
def get_contact(message: Message) -> None:
    if message.content_type == 'contact':
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            data['telephone_number'] = message.contact.phone_number
            text = f'Спасибо за информацию, ваши данные: \n' \
                   f'имя - {data["name"]}\n возраст - {data["age"]}\n' \
                   f'страна - {data["contry"]} \n номер телефона - {data["telephone_number"]}'
